...
Interval values serialised in VOTable (\verb|FIELD|) or service parameters
(\verb|PARAM|) with this xtype may include additional metadata like minimum and
...
or maximum value. These are specified using the a custom interpretation of the
...
and/or maximum values. These are specified using the a custom interpretation of the
...
\subsubsection{MOC}
MOC (Multi Order Coverage) values serialised in VOTable or service parameters must 
have the following metadata in the \xmlel{FIELD} element: \verb|datatype="char"|, 
\verb|arraysize="*"|, \verb|xtype="moc"|
(where arraysize may also be fixed length or variable length with limit).
The value is the string serialisation specified in \citet{2019ivoa.spec.1007F}.
...
coordinates and radius, it is useful for describing parameters of a request related
to a specific target location (for example, a SODA cutout of specific archival data).
The type label used in the maximum shape only tells the client how to interpret the
value; it does not limit the caller to only using that type of shape.
