...
\emph{point}, \emph{circle}, \emph{polygon}, \emph{shape}, \emph{multipolygon}, 
and \emph{moc} (see below). 
Services may 
...
\subsubsection{Shape}
Shape values serialised in VOTable or sevice parameters must have the following metadata in the 
\xmlel{FIELD} element: \verb|datatype="char"|, \verb|arraysize="*"|, \verb|xtype="shape"|. The value
is a polymorphic shape made up of a type label (equivalent to an existing xtype: \verb|circle| 
or \verb|polygon|) and the string serialisation of the value as described above. For example:
...
MOC (Multi Order Coverage) values serialised in VOTable or service parameters must have the following metadata in the \xmlel{FIELD} element:  \verb|datatype="double"| or \verb|datatype="float"|, \verb|arraysize="*"|,
\verb|xtype="multipolygon"|. The array holds a sequence of non-overlapping polygon(s) separated by a \verb|NaN|
value. For example:

\begin{verbatim}
10.0 10.0 10.2 10.0 10.2 10.2 10.0 10.2 NaN 11.0 11.0 11.2 11.0 11.2 11.2 11.0 11.2
\end{verbatim}
...
\subsubsection{MOC}
MOC (Multi Order Coverage) values serialised in VOTable or service parameters must have the following metadata in the \xmlel{FIELD} element: \verb|datatype="char"|, \verb|arraysize="*"|, \verb|xtype="moc"|. 
The value is the string serialisation specified in \citep{std:MOC}.
