...
\emph{multipolygon}, and \emph{shape} (see below). 
Services may 
...
(\xmlel{PARAM}) with this xtype may include additional metadata like minimum and
...
\subsubsection{Sexigessimal Coordinates}
Coordinate values expressed in sexigessimal form can be described using the following
...
For xtype="hms", the values are serialised as hours:minutes:seconds where hours
and minutes are integer values and seconds is a real value. For xtype="dms", the values
...
\subsubsection{Range}
Range values are coordinate bounding boxes specified as two intervals: interval of longitude
and interval of latitude. When serialised in VOTable or service parameters, a range must
have the following metadata in the \xmlel{FIELD} element: \verb|datatype="double"|
or \verb|datatype="float"|, \verb|arraysize="4"|, \verb|xtype="range"|. For ranges
in a spherical coordinate system, the values are ordered as:
min-longitude max-longitude min-latitude max-latitude. For example:

\begin{verbatim}
10.0 11.0 20.0 21.0
\end{verbatim}

includes longitude from 10 to 11 and latitude from 20 to 21 (degrees) in spherical
coordinates. In spherical coordinates, all longitude values must fall within [0,360]
and all latitude values within [-90,90]. This range form is used as part of the value
of the POS parameter in \citep{2015ivoa.spec.1223D} and \citep{2017ivoa.spec.0517B}
(see "shape" below).
