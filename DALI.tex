...
string representation of the Space-Time Coordinates \citep{note:STC} data 
model. The 
...
A registry extension schema, usually extending \citep{std:VODS11}, is needed 
to 
...
VODataService-1.1 \citep{std:VODS11} standard and allows the service to 
...
FITS \citep{std:FITS} and STC \citep{note:STC}:
...
described in \citep{std:VOTABLE}. All date-time values (formatted sting, JD, 
and MJD) shall be 
interpreted as referring to time scale UTC and time reference position UNKNOWN, 
unless either or both of these are explicitly specified to be different 
\citep{note:STC}.
...
follows the IVOA version number scheme \citep{std:DocSTDProc}. Standards at 
the Working Draft (WD) 
...
values that are content-types \citep{std:MIME} for known 
formats as well as 
...
VOTable document as described in \citep{std:VOTABLE} or a plain text document. 
The content of 
