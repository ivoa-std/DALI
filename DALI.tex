...
\lstinputlisting[language=XML,basicstyle=\footnotesize]{uws-example.xml}
...
As of DALI-1.2, the definitions of current standard parameters (below) include an
OpenAPI component that specifies correct case; the OpenAPI component is definitive.
DALI strongly recommends that new parameters be defined for consistency with existing 
text and style where possible, which in most cases will mean upper-case parameter
names. In the examples above, exactly one of those is ``correct'' and the other 
``spellings'' are not (see section~\ref{sec:unknown-parameters}).
...
case as specified by standards as soon as possible, ideally as soon as the OpenAPI
description becomes available (e.g. in a proposed recommendation).

Existing services continue to support case-insensitive parameter names
(``legacy'' parameters) until they are upgraded to a new version of a
standard with an OpenAPI specification and case-sensitive parameter names, after
which they should only support the specified case.

We expect that DALI-compliant standards will make this change to include an OpenAPI
specification and thus case-sensitive parameter names in the next minor version. We
also expect that old and unmaintained clients are most likely used with existing services
and would not be used with new services implementing only case-sensitive parameter handling.
While this change seems abrupt and strict, fixing clients is simple and a more gradual
transition does not seem feasible.

Service validation must be able to differentiate between minor versions in order to
check parameter name handling.
...
\subsection{Deprecated and Unknown Parameters}
...
self-document the query response). 

We recommend that services follow the advice in Data Origin \citep{2026ivoa.spec.0331L}
to add additional metadata when it is appropriate and feasible. Clients should not depend
on these and validators should not issue warnings if they are not present.

Prior versions of DALI defined the following:
...
There is currently an inconsistency between DALI and standard IVOA terminology and the
Data Origin note with respect to \verb|standardID|, where it is labelled as \verb|service\_protocol|
in the latter.  Example use:
\begin{lstlisting}[language=XML]
<RESOURCE type="results">
...
...
\end{lstlisting}
DALI recommends continuing to use \verb|standardID|.

Prior versions of DALI did not define the content for \verb|citation| and also recommended
some specific UCD attributes; this is deprecated in favour of details from the Data Origin note.
...
\item deprecate REQUEST and RUNID parameters
...
``endpoint''
\item add reference to EN-Data Origin for additional info in output VOTables
\end{itemize}
