...
a DALI-async resource that accepts one or more UPLOAD parameters 
...
a VOSI-availability resource (section~\ref{sec:vosi-availability}) 

a VOSI-capabilities resource (section~\ref{sec:vosi-capabilities}) conforming 
...
resources must be siblings of the VOSI-capabilities resource; this 
limitation enables a client with just the URL for a single resource to 
...
etc. We define some general purpose values for the xtype attribute of 
the VOTable FIELD and PARAM elements  for simple structured values: interval, 
point, circle, and polygon (see below).
...
described in \citep{std:VOTABLE}. All date-time values (formatted string, JD, 
...
Timestamp values serialised on VOTable must have the following metadata in 
the FIELD element: datatype="char" arraysize="*" xtype="timestamp".
 
\subsubsection{Intervals}
Numeric intervals are pairs of floating-point values (float or double). The 
bounding values are always included in the interval. Open-ended intervals have 
one or both bounding values that are infinite. For example:
...
Are all legal numeric interval values. 

Interval values serialised in VOTable must have the following metadata in the 
FIELD element: datatype="double" arraysize="2" xtype="interval".
...
Point values serialised in VOTable must have the following metadata in the 
FIELD element: datatype="double" arraysize="2" xtype="point". For points in a 
...
Circle values serialised in VOTable must have the following metadata in the 
FIELD element: datatype="double" arraysize="3" xtype="circle". For circles 
...
Polygon values serialised in VOTable must have the following metadata in the 
FIELD element: datatype="double" arraysize="*" xtype="polygon". The 
...
Option 1: The vertices of a polygon must be specified in counter-clockwise 
order.

Option 2: The vertices of a polygon may be specified in clockwise or 
counter-clockwise order. The inside is the region with the smallest area;  
polygons in spherical coordinates are limited to less than half the sphere.

\subsubsection{More geometry?}
Ellipse? Coord-range? STC Box?
...
(response documnets like VOTable) of services consistent. Added specification 
