...
that creates a job or it may add additional paramaters by a POST to the current 
...
element. For the generic-parameter, we also define a value for the
\xmlel{typeof} 
...
\noindent are all legal numeric interval values. 

Interval values serialised in VOTable must have the following metadata in the 
\xmlel{FIELD} element: \verb|datatype="double"|, \verb|arraysize="2"|,
\verb|xtype="interval"|.
...
\xmlel{FIELD} element: \verb|datatype="double"|, \verb|arraysize="3"|,
...
\xmlel{FIELD} element: \verb|datatype="double"|, \verb|arraysize="*"|,
...
\todo{Choose one of these options}
...
\xmlel{INFO}
element in the \xmlel{RESOURCE} with \texttt{name="QUERY\_STATUS"} and the 
...
\verb|name="QUERY\_STATUS"| and the \verb|value="ERROR"|. If the initial info element (above) 
